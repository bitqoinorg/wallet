...
const en: Translations = {
  ...
  proof: {
    ...
    copyKey: 'Copy Key 1', vaultLabel: 'Live Qoin Balance', loading: 'Loading...', refresh: 'Refresh',
    viewExplorer: 'View on Solana Explorer', noTokens: 'No tokens in this Qoin yet.',
  },
  how: {
    badge: 'Three Steps', title: 'Upgrade Your Wallet in 60 Seconds',
    body: 'No account needed. No server. No third party. Just keys, math, and the blockchain. Solana and Ethereum.',
    ...
    step3title: 'All Keys or Nothing', step3desc: 'To move any token, every registered key must sign. Miss one? The blockchain rejects the transaction before it executes. No exceptions.', step3note: 'Protocol-level enforcement. Not a UI checkbox.',
  },
  why: {
  ...
  generate: {
    title: 'Create Your Qoin', subtitle: 'Generate Qonjoint keys and register your Qoin on-chain. Both keys must sign every transaction. The math enforces it.',
    ...
    successTitle: 'Qoin Activated.', successSub: 'Your Qonjoint Qoin is live on-chain.',
  ...
  subpages: {
    ...
    solution: {
      ...
      p1: 'Qoin uses the Qonjoint protocol to require multiple independent keys for every transaction. No single key can move anything. The blockchain enforces this at runtime, not at the software level.',
    ...
    gettingStarted: {
      ...
      p1: 'Creating a Qoin takes two steps: generate two independent keys locally in your browser, then activate the Qonjoint protection on-chain. BITQ covers all gas fees during beta, no SOL or ETH needed. No account. No KYC. No server.',
    ...
    hackChallenge: {
      ...
      body: 'We built our own Qoin as proof of concept and published Key 1 publicly. It is right here. You can copy it, import it, sign with it. You still cannot move a single token. Key 2 is required. The Qonjoint protocol rejects any transaction missing a signature from both keys.',
    },
    liveBalance: {
      badge: 'Live Balance', title: 'Real Tokens. Real Blockchain. No Middleware.',
      subtitle: 'Live on-chain. No server. No middleware.',
      body: 'The balance shown below is fetched directly from the blockchain via RPC. No server. No caching. No middleware. Your browser talks directly to the blockchain and reads what is there. This is how Qoin works for your own wallet too.',
    },
    faqPage: {
...
const ja: Translations = {
  ...
  proof: {
    ...
    liveTitle: 'ライブQoin残高', liveSub: 'オンチェーンで直接確認。サーバーなし。ミドルウェアなし。',
    ...
    copyKey: '鍵1をコピー', vaultLabel: 'ライブQoin残高', loading: '読み込み中...', refresh: '更新',
    viewExplorer: 'Solanaエクスプローラーで確認', noTokens: 'このQoinにはまだトークンがありません。',
  },
  how: {
    badge: '3ステップ', title: '60秒でウォレットをアップグレード',
    body: 'アカウント不要。サーバーなし。第三者なし。鍵と数学とブロックチェーンだけ。SolanaとEthereum対応。',
    ...
    step3title: '全鍵か無か', step3desc: 'トークンを動かすには、登録された全鍵が署名しなければなりません。1つでも欠けると？ブロックチェーンが実行前に拒否します。例外なし。', step3note: 'プロトコルレベルの強制。UIのチェックボックスではありません。',
  },
  why: {
  ...
  faq: {
    ...
    items: [
      ...
      { q: 'どのトークンを保護できますか？', a: 'SolanaのあらゆるSPLトークン、またはEthereumのERC-20トークン。ミームコイン、ステーブルコイン、NFT、何でも。オンチェーンにあれば、Qoinで保持できます。' },
  ...
  generate: {
    title: 'Qoinを作成', subtitle: 'Qonjoint鍵を生成し、QoinをオンチェーンにQonjoint登録します。すべての取引に両方の鍵の署名が必要です。',
    ...
    successTitle: 'Qoin有効化完了。', successSub: 'QonjointのQoinがオンチェーンで稼動しています。',
  ...
  subpages: {
    ...
    solution: { badge: 'Qoinの解決策', title: '2つの鍵。1つのプロトコル。妥協なし。', p1: 'QoinはQonjointプロトコルを使用し、すべての取引に複数の独立した鍵を要求します。単一の鍵は何も動かせません。', p2: '鍵はブラウザでローカルに生成されます。サーバーには触れません。私たちには見えません。', p3: '中間に会社は存在しません。ブロックチェーンによって強制される数学です。', handwritten: 'ソフトウェア保護ではなく、プロトコル保護。' },
    ...
    gettingStarted: { badge: 'はじめに', title: '60秒以内に起動', p1: 'Qoinの作成は2ステップです。ブラウザでローカルに2つの独立した鍵を生成し、オンチェーンでQonjoint保護を有効化します。ガス代はベータ期間中BITQが負担。SOLもETHも不要。アカウント不要。KYC不要。', handwritten: 'あなたの鍵。あなたのトークン。あなたの数学。' },
    protocol: { badge: 'Qonjointプロトコル', title: 'オンチェーン保護の仕組み', p1: 'QonjointはSolanaとEthereumをサポートします。Qoinを作成すると、2つの公開鍵がオンチェーンにQonjointコントローラーとして登録されます。', p2: 'この強制はプロトコルレベルで発生します。フロントエンドが完全に侵害または停止されても、トークンを動かすには両方の鍵が必要です。', handwritten: 'オンチェーン。検証可能。永続的。' },
...
const zh: Translations = {
  ...
  proof: {
    ...
    liveTitle: '实时Qoin余额', liveSub: '直接链上验证。无服务器。无中间件。',
    fetchingRpc: '链上数据获取中...',
    solBalance: 'SOL余额', holdings: 'bitQoin持仓', qoinjointLock: 'Qonjoint锁定',
    qoinAddress: 'Qoin地址:', verifyBtn: '在Orb上自行验证', reloadBtn: '重新加载余额',
    copyKey: '复制密钥1', vaultLabel: '实时Qoin余额', loading: '加载中...', refresh: '刷新',
    viewExplorer: '在Solana浏览器中查看', noTokens: '此Qoin中尚无代币。',
  },
  how: {
    badge: '三个步骤', title: '60秒内升级您的钱包',
    body: '无需账户。无需服务器。无第三方。只需密钥、数学和区块链。支持Solana和Ethereum。',
    ...
    step3title: '全部密钥或没有', step3desc: '要移动任何代币，每个已注册的密钥都必须签名。少一个？区块链在执行前拒绝交易。无例外。', step3note: '协议级执行。不是UI复选框。',
  },
  why: {
  ...
  faq: {
    ...
    items: [
      ...
      { q: '我可以保护哪些代币？', a: 'Solana上的任何SPL代币，或Ethereum上的ERC-20代币。Meme币、稳定币、NFT，任何东西。只要它在链上，就可以在Qoin中持有。' },
      { q: '需要SOL支付Gas费吗？', a: '不需要。BITQ在测试期间承担所有Gas费用。无需持有SOL或ETH即可免费创建和使用Qoin。' },
      { q: '可以在手机上使用吗？', a: '可以。该应用完全基于浏览器。任何具有现代浏览器的设备都可以使用。' },
    ],
  },
  generate: {
    title: '创建您的Qoin', subtitle: '生成Qonjoint密钥并在链上注册您的Qoin。每笔交易都需要两把密钥签名。数学强制执行。',
    ...
    successTitle: 'Qoin已激活。', successSub: '您的Qonjoint Qoin已在链上运行。',
  ...
  subpages: {
    ...
    solution: { badge: 'Qoin解决方案', title: '两把密钥。一个协议。零妥协。', p1: 'Qoin使用Qonjoint协议要求每笔交易有多个独立密钥。没有任何单个密钥可以移动任何东西。', p2: '您的密钥在浏览器中本地生成。它们从不接触服务器。我们永远看不到它们。', p3: '中间没有公司。这是由区块链强制执行的数学。', handwritten: '不是软件保护。是协议保护。' },
    ...
    gettingStarted: { badge: '入门指南', title: '60秒内启动运行', p1: '创建Qoin只需两步：在浏览器中本地生成两个独立密钥，然后在链上激活Qonjoint保护。BITQ在测试期间承担所有Gas费用，无需SOL或ETH。无需账户。无需KYC。', handwritten: '您的密钥。您的代币。您的数学。' },
    protocol: { badge: 'Qonjoint协议', title: '链上保护如何运作', p1: 'Qonjoint支持Solana和Ethereum。当您创建Qoin时，两个公钥在链上注册为Qonjoint联合控制者。', p2: '这种强制在协议层面发生。即使我们的前端被完全攻破，您的代币仍需要两把密钥才能移动。', handwritten: '链上。可验证。永久。' },
...
const de: Translations = {
  ...
  proof: {
    ...
    liveTitle: 'Live Qoin-Guthaben', liveSub: 'Direkt on-chain verifiziert. Kein Server. Keine Middleware.',
    ...
    copyKey: 'Schlüssel 1 kopieren', vaultLabel: 'Live-Qoin-Guthaben', loading: 'Wird geladen...', refresh: 'Aktualisieren',
    viewExplorer: 'Im Solana Explorer anzeigen', noTokens: 'Noch keine Token in diesem Qoin.',
  },
  how: {
    badge: 'Drei Schritte', title: 'Wallet in 60 Sekunden upgraden',
    body: 'Kein Konto erforderlich. Kein Server. Kein Dritter. Nur Schlüssel, Mathematik und die Blockchain. Solana und Ethereum.',
    ...
    step3title: 'Alle Schlüssel oder nichts', step3desc: 'Um einen Token zu bewegen, muss jeder registrierte Schlüssel signieren. Fehlt einer? Die Blockchain lehnt die Transaktion vor der Ausführung ab. Keine Ausnahmen.', step3note: 'Protokoll-Ebene Durchsetzung. Kein UI-Kontrollkästchen.',
  },
  why: {
  ...
  faq: {
    ...
    items: [
      ...
      { q: 'Welche Tokens kann ich schützen?', a: 'Jeden SPL-Token auf Solana oder ERC-20-Token auf Ethereum. Memecoins, Stablecoins, NFTs, alles. Wenn es on-chain lebt, kann es in einem Qoin gehalten werden.' },
      { q: 'Brauche ich SOL für Gasgebühren?', a: 'Nein. BITQ übernimmt alle Gasgebühren während der Beta. Erstelle und nutze deinen Qoin kostenlos, kein SOL oder ETH für Transaktionen nötig.' },
  ...
  generate: {
    title: 'Dein Qoin erstellen', subtitle: 'Generiere Qonjoint-Schlüssel und registriere deinen Qoin on-chain.',
    ...
    successTitle: 'Qoin aktiviert.', successSub: 'Dein Qonjoint Qoin ist on-chain live.',
...
const es: Translations = {
  ...
  proof: {
    ...
    liveTitle: 'Balance en Vivo de Qoin', liveSub: 'Verificado en vivo on-chain. Sin servidor. Sin middleware.',
    ...
    copyKey: 'Copiar Clave 1', vaultLabel: 'Saldo Qoin en Vivo', loading: 'Cargando...', refresh: 'Actualizar',
    viewExplorer: 'Ver en Solana Explorer', noTokens: 'No hay tokens en este Qoin aún.',
  },
  how: {
    badge: 'Tres Pasos', title: 'Actualiza tu Wallet en 60 Segundos',
    body: 'Sin cuenta. Sin servidor. Sin terceros. Solo claves, matemáticas y la blockchain. Solana y Ethereum.',
    ...
    step3title: 'Todas las Claves o Nada', step3desc: 'Para mover cualquier token, todas las claves registradas deben firmar. ¿Falta una? La blockchain rechaza la transacción. Sin excepciones.', step3note: 'Aplicación a nivel de protocolo. No una casilla de verificación.',
  },
  why: {
  ...
  faq: {
    ...
    items: [
      ...
      { q: '¿Qué tokens puedo proteger?', a: 'Cualquier token SPL en Solana, o token ERC-20 en Ethereum. Memecoins, stablecoins, NFTs, cualquier cosa. Si vive on-chain, puede guardarse en un Qoin.' },
      { q: '¿Necesito SOL para las tarifas de gas?', a: 'No. BITQ cubre todas las tarifas de gas durante la beta. Crea y usa tu Qoin gratis, no necesitas SOL ni ETH para las transacciones.' },
  ...
  generate: {
    title: 'Crear tu Qoin', subtitle: 'Genera claves Qonjoint y registra tu Qoin on-chain.',
    ...
    successTitle: 'Qoin Activado.', successSub: 'Tu Qoin Qonjoint está en vivo on-chain.',
  ...
  subpages: {
    ...
    solution: { badge: 'La Solución Qoin', title: 'Dos Claves. Un Protocolo. Cero Compromisos.', p1: 'Qoin usa el protocolo Qonjoint para requerir múltiples claves independientes en cada transacción. Ninguna clave única puede mover nada.', p2: 'Tus claves se generan localmente en tu navegador. Nunca tocan un servidor.', p3: 'No hay empresa en el medio. Es matemática, aplicada por la blockchain.', handwritten: 'No es protección de software. Es protección de protocolo.' },
...
const ar: Translations = {
  ...
  proof: {
    ...
    liveTitle: 'رصيد Qoin المباشر', liveSub: 'تم التحقق مباشرة on-chain. لا خادم. لا وسيط.',
    ...
    copyKey: 'نسخ المفتاح 1', vaultLabel: 'رصيد Qoin المباشر', loading: 'جارٍ التحميل...', refresh: 'تحديث',
    viewExplorer: 'عرض في Solana Explorer', noTokens: 'لا توجد رموز في هذا Qoin بعد.',
  },
  how: {
    badge: 'ثلاث خطوات', title: 'ترقية محفظتك في 60 ثانية',
    body: 'لا حاجة لحساب. لا خادم. لا طرف ثالث. فقط مفاتيح ورياضيات والبلوكشين. Solana وEthereum.',
    ...
    step3title: 'جميع المفاتيح أو لا شيء', step3desc: 'لنقل أي رمز، يجب أن يوقع كل مفتاح مسجل. تفقد واحدا؟ البلوكشين يرفض المعاملة قبل التنفيذ. لا استثناءات.', step3note: 'تطبيق على مستوى البروتوكول. ليس مربع تحقق في الواجهة.',
  },
  why: {
  ...
  faq: {
    ...
    items: [
      ...
      { q: 'ما الرموز التي يمكنني حمايتها؟', a: 'أي رمز SPL على Solana، أو ERC-20 على Ethereum. عملات الميم، العملات المستقرة، NFTs، أي شيء. إذا كان يعيش on-chain، يمكن الاحتفاظ به في Qoin.' },
      { q: 'هل أحتاج SOL لرسوم الغاز؟', a: 'لا. BITQ يغطي جميع رسوم الغاز خلال الفترة التجريبية. أنشئ واستخدم Qoin مجانا، لا تحتاج SOL ولا ETH للمعاملات.' },
      { q: 'هل يمكنني استخدامه على الجوال؟', a: 'نعم. التطبيق مبني بالكامل على المتصفح. أي جهاز بمتصفح حديث يعمل.' },
    ],
  },
  generate: {
    title: 'إنشاء Qoin الخاص بك', subtitle: 'أنشئ مفاتيح Qonjoint وسجل Qoin الخاص بك on-chain.',
    ...
    successTitle: 'تم تفعيل Qoin.', successSub: 'Qoin Qonjoint الخاص بك يعمل الآن on-chain.',
  ...
  subpages: {
    ...
    solution: { badge: 'حل Qoin', title: 'مفتاحان. بروتوكول واحد. صفر تنازلات.', p1: 'يستخدم Qoin بروتوكول Qonjoint لمطالبة مفاتيح مستقلة متعددة لكل معاملة. لا يمكن لأي مفتاح واحد تحريك أي شيء.', p2: 'مفاتيحك تُولد محليا في متصفحك. لا تلمس خادما أبدا.', p3: 'لا توجد شركة في المنتصف. إنها رياضيات يُطبقها البلوكشين.', handwritten: 'ليست حماية برمجية. حماية بروتوكول.' },
    ...
    gettingStarted: { badge: 'البداية', title: 'جاهز للعمل في أقل من 60 ثانية', p1: 'إنشاء Qoin يتطلب خطوتين: توليد مفتاحين مستقلين محليا في متصفحك، ثم تفعيل حماية Qonjoint على السلسلة. BITQ يغطي جميع رسوم الغاز خلال البيتا، لا SOL ولا ETH مطلوب. بلا حساب. بلا KYC. بلا خادم.', handwritten: 'مفاتيحك. رموزك. رياضياتك.' },
    protocol: { badge: 'بروتوكول Qonjoint', title: 'كيف تعمل الحماية على السلسلة', p1: 'يدعم Qonjoint Solana وEthereum. عند إنشاء Qoin، يُسجل مفتاحان عاميان on-chain كمتحكمين Qonjoint مشتركين.', p2: 'هذا التطبيق يحدث على مستوى البروتوكول. حتى لو تعرضت واجهتنا للاختراق، رموزك تتطلب كلا المفتاحين للتحرك.', handwritten: 'على السلسلة. قابل للتحقق. دائم.' },
